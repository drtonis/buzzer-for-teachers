...
# BORDER_COLOR = pygame.Color(200, 200, 200)
...
while not done:
    for event in pygame.event.get():
        ...
        if event.type == pygame.MOUSEBUTTONDOWN:
            ...
            # if ok_button.collidepoint(event.pos) and text.strip():
            #     tts = gTTS(text=text, lang=selected_language, tld=selected_accent, slow=False)
            #     tts.save(f"000{selected_file_number}.mp3")
            #     text = ''
            # if clear_button.collidepoint(event.pos):
            #     text = ''
            # if test_button.collidepoint(event.pos) and text.strip():
            #     tts = gTTS(text=text, lang=selected_language, tld=selected_accent, slow=False)
            #     tmp_file = tempfile.NamedTemporaryFile(delete=False, suffix=".mp3")
            #     tmp_file.close()
            #     tts.save(tmp_file.name)
            ...
            #     else:
            #         dropdown_open = False
            # if file_dropdown_rect.collidepoint(event.pos):
            #     file_dropdown_open = not file_dropdown_open
            # elif file_dropdown_open:
            #     for i, num in enumerate(file_numbers):
            #         option_rect = pygame.Rect(file_dropdown_rect.x, file_dropdown_rect.y - (i + 1) * 30, file_dropdown_rect.width, 30)
            #         if option_rect.collidepoint(event.pos):
            #             selected_file_number = num
    ...
    ok_text = button_font.render("Save", True, BUTTON_TEXT_COLOR)
